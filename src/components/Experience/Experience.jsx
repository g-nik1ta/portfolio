...
const Experience = () => {
    ...
    const companiesCommon = useSelector(
        (state) => state.ExperienceReducer.companies
    );
    const companies_translations = useSelector(
        (state) => state.ExperienceReducer.companies_translations
    );

    const companies = useTranslationDataArray(
        companiesCommon,
                ...
                {companies.map((company) => (
                    <div key={company.id} className="experience_item">
                        <div className="info_card">
                            <div className="head">
                                ...
                                >
                                    {company.logo ? (
                                        <img
                                            src={require(`assets/companies/${company.logo}`)}
                            ...
                            {company.translation.achievements.length > 0 && (
                                <div className="achievements">
                                    <b>{t("experience.achievements")}:</b>
                                    <ul className="undecorated_ul">
                                        {company.translation.achievements.map(
                                            (achievement, index) => (
                                                <li key={index}>
                                                    {achievement}
                                                </li>
                        ...
                        <div className="logo-date flex">
                            ...
                            <span className="date hide-if_lg_size">
                                {company.translation.date}
                            </span>
                        </div>
                    </div>
