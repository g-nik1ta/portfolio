...
import USlider from "components/UI/USlider/USlider";
...
const ModalProject = () => {
    ...
    const currentProject = useSelector(
        (state) => state.ProjectsReducer.currentProject
    );
    const projectsCommon = useSelector(
        (state) => state.ProjectsReducer.projects
    );
    const projects_translations = useSelector(
        (state) => state.ProjectsReducer.projects_translations
    );
    ...
    useEffect(() => {
        ...
        document
            .querySelector(".modal_wrapper.modal-project")
        ...
        <Modal modalName="modal-project" closeHandler={closeHandler}>
            {project?.images?.length > 0 ? (
                <USlider
                    ...
                    project_name={project.translation.project_name}
                />
            ) : (
                <div className="img_wrapper">
                    <img
                        src={require(`assets/projects/${project.preview}`)}
                        alt={`${project.translation.project_name} preview`}
                    />
                </div>
            )}
            <h2 className="project_name">{project.translation.project_name}</h2>
            <h4 className="duration_work">
                {project.translation.duration_work}
            </h4>
            ...
            <p className="project_description">
                {project.translation.project_description}
            </p>
            {project.appHref && (
